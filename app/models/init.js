...
module.exports = function(model) {
  ...
  .declare('indexDescription', function(req, fill) {
    var index = {
      ...
      "_rootSince": new Date(req.config.get('defaultRootCollection') === req.config.get('collectionNameIndex') ? Date.now() : 0),
  ...
  .declare('hotfolderDescription', function(req, fill) {
    ...
    if (req.config.has('dataPath')) {
      index = {
        ...
        "_rootSince": new Date(req.config.get('defaultRootCollection') === req.config.get('collectionNameHotFolder') ? Date.now() : 0),
        "_updated": new Date(Date.now())
      };
    }
    fill(index);
