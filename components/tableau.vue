	...
	<table class="table-bordered table-striped table-condensed cf">
		...
		<tbody>
		  <tr v-for="(index, doc) in items">
				<td data-title="URI">
					<a href="{{ doc._uri }}?alt=html">{{ doc._wid }}</a>
				</td>
				<td v-for="(colkey, col) in columns">
					{{{ doc._columns[colkey].content.raw }}}
				</td>
			</tr>
		</tbody>
	</table>
...
export default {
	...
	methods: {
		reload(done) {
			...
			Object.keys(self.sortKeys).filter(function(a) { return self.sortKeys[a] !== 0 }).forEach(function(b) {qry.$sort['_columns.'+ b +'.content'] = self.sortKeys[b]})
			...
			self.$http.get(url).then(function (response) {
				if (Object.keys(self.columns).length === 0) {
					...
					.filter(function(k) { return response.body[0]._columns[k].displayAreas && response.body[0]._columns[k].displayAreas.table === true })
